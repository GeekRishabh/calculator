...
export default class Calculator extends Component {
  ...
  render() {
    return (
      <View style={Style.rootContainer}>
        ...
        <View style={Style.inputContainer}>{this._renderButtons()}</View>
      </View>
    );
  }

  // For each row in `buttons`, create a row View and add create
  // Button for each input in the row.
  _renderButtons() {
    let views = [];
    //for loop over the array of buttons
      ...
      for (var i = 0; i < row.length; i++) {
        ...
        inputRow.push(
          <Button
            ...
            onPress={this._onButtonPressed.bind(this, input)}
  ...
  _onButtonPressed(input) {
    ...
    // if number or the input buttons
    switch (typeof input) {
      case "number":
  ...
  _handleNumberInput(num) {
    ...
    this.setState({
      ...
      inputValue: this.state.isDecimal
        ? eval(this.state.currentInputValue + this.state.selectedSymbol + num)
        : this.state.inputValue * 10 + num,
      currentInputValue: this.state.isDecimal
        ? 0
        : this.state.inputValue * 10 + num,
      connectValue: null,
      displayedValue: null,
